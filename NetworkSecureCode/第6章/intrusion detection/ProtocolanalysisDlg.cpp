...
extern char Global_FilePath[_MAX_PATH];
...
void CProtocolAnalysisDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_ETHERNET, m_list_ethernet);
	...
	DDX_Control(pDX, IDC_LIST_INFORMATION, m_list_common);
	//}}AFX_DATA_MAP
}
...
ON_COMMAND(ID_MENU_OPEN_DUMP_FILE, OnMenuOpenDumpFile)
	...
	ON_COMMAND(ID_MENU_INTRUSION, OnMenuIntrusion)
ON_WM_NCPAINT()
...
{	
	...
	m_list_tcp.InsertColumn (11,"tcp数据",LVCFMT_LEFT, 300, 1);
	...
	m_list_tcp.SetColumnWidth(11, 110);
	...
	m_list_udp.InsertColumn(5, "UDP数据", LVCFMT_LEFT, 300, 1);
	...
	m_list_udp.SetColumnWidth(5, 120);
	...
	m_list_intrusion.SetColumnWidth(1, 120);
	m_list_intrusion.SetColumnWidth(2, 120);
	m_list_intrusion.SetColumnWidth(3, 120);
	m_list_intrusion.SetColumnWidth(4, 120);
	...
	m_list_intrusion.SetWindowPos(NULL, rect1.left, rect1.top, rect1.Width(),
		rect1.Height() - rect2.Height() - 000, NULL); 
	m_list_intrusion.ShowWindow(SW_HIDE);
	hWnd = GetSafeHwnd();
	SetWindowText("网络入侵检测系统《网络安全编程技术与实例》（作者：刘文涛）");  
...
void CProtocolAnalysisDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	{
		int result =MessageBox("\n\n你真的要退出吗？","网络入侵检测系统《网络安全编程技术与实例》（作者：刘文涛）",MB_OKCANCEL);
		if(result==IDOK)
			PostQuitMessage(1);
...
LRESULT CProtocolAnalysisDlg::OnTcp(WPARAM wParam, LPARAM lParam)
{
	...
	m_list_tcp.SetItemText (0,11,TcpData);
	PacketNumber.tcp = i;
	i++;  
	return  0;
}
LRESULT CProtocolAnalysisDlg::OnUdp(WPARAM wParam, LPARAM lParam)
{
	...
	m_list_udp.SetItemText (0,5,UdpData);
	PacketNumber.udp = i;
	i++;  
	return  0;
}
...
LRESULT CProtocolAnalysisDlg::OnIntrusion(WPARAM wParam, LPARAM lParam)
{
	static i = 1;  
	char str[10]; 
	sprintf(str, "%d", i);
...
void CProtocolAnalysisDlg::OnSelchangeTab1(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	{	
		m_list_intrusion.ShowWindow(SW_SHOW);
		m_list_icmp.ShowWindow(SW_HIDE);
		m_list_ip.ShowWindow(SW_HIDE);
		m_list_common.ShowWindow(SW_HIDE);
		m_list_arp.ShowWindow(SW_HIDE);
...
void CProtocolAnalysisDlg::OnMenuIntrusion() 
{
	m_tab1.SetCurSel (7);
	ShowTabWindow();
	CStatic *p=(CStatic *)GetDlgItem(IDC_STATIC_STATUS);
